...
- **Search modal** — one modal that jumps and searches: fuzzy-match a note by display title, alias, or path and open it, or take the pinned bottom row into the full-text page. Each match shows what matched — a small `title:`/`alias:`/`file:`/`path:` line with the matched characters highlighted under the row (filename hits outrank folder hits); top 10 results. `Ctrl/Cmd+1–9` jumps straight to a result, `Ctrl/Cmd+Enter` opens the full search with the query. Type `#` to complete tags; empty query shows this session's recently-opened notes.
