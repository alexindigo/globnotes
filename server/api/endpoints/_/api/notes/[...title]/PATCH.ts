...
export default async function (ctx: RequestCtx) {
  ...
  if (data.newTitle !== undefined && data.newTitle !== null) {
    data.newTitle = validateNoteTitle(data.newTitle, "newTitle");
  }
  try {
    return state.notes.update(ctx.params.title, data, fileRefs);
  } catch (e) {
